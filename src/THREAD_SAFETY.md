...
- **Diagnostics and errors:** Use `log_write(kind, format, ...)` from [log.h](log.h):
  ...
  - **DEBUG_CLI**: Kind label for mirrored [`BarUiMsg`](ui.c) lines on stderr. Emits when **any** `PIANOBAR_DEBUG` bit is non-zero (not only bit 32); only in **web** / **both** UI modes. Do not call `log_write(DEBUG_CLI, …)` by hand except from `BarUiMsg` unless you intentionally match that format.
